...
export const createActividad = (grupoId: number, data: { unidadId: number; nombre: string; ponderacion: number }) =>
  client.post<ActividadGrupoResponse>(base(grupoId), data).then(r => r.data)
export const updateActividad = (grupoId: number, id: number, data: { nombre: string; ponderacion: number }) =>
