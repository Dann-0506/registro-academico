...
const emptyForm = {
  ...
  materiaId: '',
  maestroId: '',
  calificacionMinimaAprobatoria: '60',
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 z-20 mt-1 w-52 bg-white rounded-xl border border-slate-200 shadow-lg py-1 text-sm">
            {grupo.estadoEvaluacion === 'ABIERTO' && (
              <button onClick={() => { setOpen(false); onCerrar() }} className="flex items-center gap-2.5 w-full px-4 py-2 text-left text-amber-700 hover:bg-amber-50 transition-colors">
                <Lock className="h-3.5 w-3.5" /> Cerrar acta
              </button>
            )}
            {grupo.estadoEvaluacion === 'CERRADO' && (
              <button onClick={() => { setOpen(false); onReabrir() }} className="flex items-center gap-2.5 w-full px-4 py-2 text-left text-blue-700 hover:bg-blue-50 transition-colors">
                <LockOpen className="h-3.5 w-3.5" /> Reabrir acta
              </button>
            )}
            <button onClick={() => { setOpen(false); onCerrarDef() }} className="flex items-center gap-2.5 w-full px-4 py-2 text-left text-red-700 hover:bg-red-50 transition-colors">
              <LockKeyhole className="h-3.5 w-3.5" /> Cerrar definitivamente
            </button>
            <div className="border-t border-slate-100 my-1" />
            <button onClick={() => { setOpen(false); onEliminar() }} className="flex items-center gap-2.5 w-full px-4 py-2 text-left text-red-600 hover:bg-red-50 transition-colors">
...
export default function Grupos() {
  ...
  const openEdit = (g: GrupoResponse) => {
    ...
    setForm({
      ...
      calificacionMinimaAprobatoria: String(g.calificacionMinimaAprobatoria),
      calificacionMaxima: String(g.calificacionMaxima),
    })
    setFormError('')
    setModalOpen(true)
  }
  ...
  return (
    <div>
      ...
      <DataTable<GrupoResponse>
        ...
        columns={[
          ...
          { header: 'Semestre', accessor: 'semestre' },
          {
            ...
            accessor: (g) => <StatusBadge estado={g.estadoEvaluacion} />,
          },
          {
            header: 'Activo',
            accessor: (g) => <StatusBadge estado={g.activo ? 'ACTIVO' : 'INACTIVO'} label={g.activo ? 'Activo' : 'Inactivo'} />,
          },
        ]}
        rowActions={(g) => (
          <div className="flex items-center justify-end gap-1">
            ...
            <ActionsDropdown
              grupo={g}
              onCerrar={() => setCerrarTarget(g)}
              onReabrir={() => setRreabrirTarget(g)}
              onCerrarDef={() => setCerrarDefTarget(g)}
              onEliminar={() => setDeleteTarget(g)}
            />
          </div>
        )}
      />
      ...
      <FormModal
        ...
        loading={isPending}
        submitLabel={editTarget ? 'Guardar cambios' : 'Crear grupo'}
        size="lg"
      >
        <div className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setForm((p) => ({ ...p, maestroId: e.target.value }))}
              className={inputClass}
            >
              <option value="">Seleccionar maestro...</option>
              {maestros.map((m) => (
                <option key={m.id} value={m.id}>{m.numEmpleado} — {m.nombre}</option>
              ))}
            </select>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm((p) => ({ ...p, calificacionMinimaAprobatoria: e.target.value }))}
                className={inputClass}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm((p) => ({ ...p, calificacionMaxima: e.target.value }))}
                className={inputClass}
              />
            </div>
          </div>
      ...
      <ConfirmDialog
        open={!!cerrarTarget}
        title="Cerrar acta del grupo"
        description={`¿Cerrar el acta del grupo "${cerrarTarget?.clave}"? Los maestros no podrán editar calificaciones hasta que se reabra.`}
        confirmLabel="Cerrar acta"
        variant="warning"
        loading={cerrarMut.isPending}
        onConfirm={() => cerrarTarget && cerrarMut.mutate(cerrarTarget.id)}
      ...
      <ConfirmDialog
        ...
        description={`¿Reabrir el acta del grupo "${reabrirTarget?.clave}"?`}
        confirmLabel="Reabrir"
        variant="default"
        loading={reabrirMut.isPending}
        onConfirm={() => reabrirTarget && reabrirMut.mutate(reabrirTarget.id)}
        onCancel={() => setRreabrirTarget(null)}
      />

      {/* Cerrar definitivamente */}
      <ConfirmDialog
        open={!!cerrarDefTarget}
        title="Cerrar definitivamente"
        description={`Esta acción cerrará el grupo "${cerrarDefTarget?.clave}" de forma permanente. No se podrá reabrir ni editar calificaciones. ¿Continuar?`}
        confirmLabel="Cerrar definitivamente"
        variant="destructive"
        loading={cerrarDefMut.isPending}
        onConfirm={() => cerrarDefTarget && cerrarDefMut.mutate(cerrarDefTarget.id)}
        onCancel={() => setCerrarDefTarget(null)}
      />
      ...
      <ConfirmDialog
        ...
        description={`¿Estás seguro de que deseas eliminar el grupo "${deleteTarget?.clave}"? Esta acción no se puede deshacer.`}
